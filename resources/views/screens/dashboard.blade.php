    ...
    <section class="page-section" data-dashboard-app data-dashboard-user-id="{{ $dashboardUserId ?? '' }}">
        <div class="hero" data-reveal>
            <div>
                <h1 class="hero__title" style="font-weight: 800; letter-spacing: -0.04em;">
                    Welcome back,<br>
                    <span style="color: var(--primary);">{{ $dashboardUserName }}</span>
                </h1>
        ...
        <div class="stats-grid">
            ...
            <article class="stat-card stagger-3" data-reveal>
                ...
                <div class="stat-card__value">{{ $dashboardStats['totals']['deck_count'] }} <small>active</small></div>
                <p class="stat-card__hint">{{ $dashboardStats['totals']['card_count'] }} cards across all systems.</p>
            </article>
        </div>
        ...
        <article class="stat-card stat-card--wide stagger-4" data-reveal>
            ...
            <div class="milestone">
                <div style="flex: 1;">
                    <div class="stat-card__value" style="font-size: 2.5rem;">{{ $dashboardStats['monthly_learned'] }} <small>/ {{ $dashboardStats['monthly_goal'] }} learned</small></div>
                ...
                <div class="progress" style="height: 12px; background: var(--surface-mid);"><div class="progress__bar" style="width: {{ $progressPercent }}%;"></div></div>
                </div>
            </div>
        </article>
        ...
        <section class="section-header" data-reveal>
            ...
            <a href="{{ route('imports.index') }}" class="primary-button primary-button--pill" style="padding: 0.6rem 1.25rem; font-size: 0.9rem;">
        ...
        <div class="deck-grid">
            @forelse ($dashboardDecks as $index => $deck)
                <article class="deck-card stagger-{{ ($index % 4) + 1 }}" data-reveal data-deck-card data-deck-id="{{ $deck['id'] }}">
                    <div class="deck-card__top">
                        ...
                        <div class="deck-card__top-actions">
                            <span class="status-badge {{ $deck['due_count'] > 0 ? 'status-badge--amber' : 'status-badge--green' }}" style="font-size: 0.7rem;">
                                {{ $deck['due_count'] > 0 ? $deck['due_count'] . ' pending' : 'On track' }}
                            </span>
                    ...
                    <div style="margin-block: 0.5rem 1rem;">
                        <h3 class="deck-card__title" style="font-size: 1.25rem;">{{ $deck['name'] }}</h3>
                    ...
                    <div class="progress-block" style="width: 100%; margin-bottom: 1.5rem;">
                        ...
                        <div class="progress" style="height: 6px; background: var(--surface-low);"><div class="progress__bar" style="width: {{ $deck['mastery_percent'] }}%"></div></div>
                    </div>
                    <div class="deck-card__actions" style="margin-top: auto; display: grid; grid-template-columns: 1fr 1.2fr; gap: 0.5rem;">
                        <a href="{{ route('decks.show', ['deck' => $deck['id']]) }}" class="secondary-button" style="padding: 0.75rem 0.5rem; font-size: 0.85rem;">View Deck</a>
                        <a href="{{ route('study.front', ['deck_id' => $deck['id']]) }}" class="primary-button" style="padding: 0.75rem 0.5rem; font-size: 0.85rem;">
        ...
        {{-- Delete Deck Modal (required by setupDashboard JS) --}}
            ...
            <table class="import-table">
                ...
                <tbody>
                    @forelse ($recentImports as $job)
                        <tr>
                            <td style="font-family: monospace; color: var(--primary);">#{{ $job['id'] }}</td>
                            <td>{{ $job['file_name'] }}</td>
                            <td>{{ $job['deck_name'] }}</td>
                            <td><span class="status-badge {{ $job['status'] === 'imported' ? 'status-badge--green' : 'status-badge--amber' }}">{{ $job['status'] }}</span></td>
                            <td><strong style="color: var(--secondary);">{{ $job['success_rows'] }}</strong></td>
                            <td><strong style="color: var(--danger);">{{ $job['failed_rows'] }}</strong></td>
                            <td class="muted-text" style="font-size: 0.85rem;">{{ $job['finished_at'] }}</td>
                        </tr>
                    @empty
